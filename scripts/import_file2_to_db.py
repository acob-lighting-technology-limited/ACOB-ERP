...
MANUAL_NAME_MAP = {
    ...
    'philip dogo': 'Philip Yakubu',
    'busayo': 'Busayo Kadiri',
    'edward': 'Edward Atoshi',
    'edward benson': 'Edward Atoshi', # Edward Atoshi in File 2
...
to_insert = [] # (user_id, date, full_name, sheet_name)
    ...
    for r in range(header_row_idx + 1, ws.max_row + 1):
        ...
        for c, dt in date_cols.items():
            ...
            if isinstance(val, (int, float)) and val > 0:
                to_insert.append((uid, dt, full_name, s_name))

print(f"Total valid meal logs extracted from File 2 for import: {len(to_insert)}")

# Fetch existing DB logs
cur.execute("SELECT user_id, date FROM attendance_lunch_log WHERE date >= '2026-01-01' AND date <= '2026-12-31';")
existing_db = set(cur.fetchall())
print(f"Existing 2026 logs in Database: {len(existing_db)}")

new_inserts = []
for uid, dt, name, s_name in to_insert:
    if (uid, dt) not in existing_db:
        new_inserts.append((uid, dt, name, s_name))

print(f"New logs to be inserted into Database: {len(new_inserts)}")
print("\nSample new logs to insert:")
for item in new_inserts[:15]:
    print(f"  Date: {item[1]} | Employee: {item[2]:<25} | Sheet: {item[3]}")

# Execute SQL Ingestion
if new_inserts:
    print(f"\nInserting {len(new_inserts)} records into attendance_lunch_log...")
    inserted_count = 0
    for uid, dt, name, s_name in new_inserts:
        cur.execute("""
            INSERT INTO attendance_lunch_log (id, user_id, date, cost, company_subsidy, employee_deduction)
            VALUES (gen_random_uuid(), %s, %s, 2200.00, 1100.00, 1100.00)
            ON CONFLICT (user_id, date) DO NOTHING;
        """, (uid, dt))
        inserted_count += cur.rowcount
    conn.commit()
    print(f"SUCCESSFULLY COMMITTED {inserted_count} new lunch records into attendance_lunch_log!")

# --- NOW RUN RECONCILIATION AGAINST FILE 1 AGAIN ---
...
print(f"\n=================================================================")
print(f"  UPDATED RECONCILIATION TABLE: FILE 1 VS UPDATED DATABASE")
print(f"=================================================================\n")

months = range(1, 8)
month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul']

print(f"{'Month':<8} | {'File 1 (ACOB Petty Cash)':<24} | {'Updated Database (DB)':<22} | {'Difference':<12}")
print("-" * 72)

for m, m_name in zip(months, month_names):
    f1_cnt = len([k for k in old_file_entries.keys() if k[0].month == m])
    db_cnt = len([k for k in updated_db_logs if k[0].month == m])
    diff = db_cnt - f1_cnt
    print(f"{m_name:<8} | {f1_cnt:<24} | {db_cnt:<22} | {diff:+d}")
