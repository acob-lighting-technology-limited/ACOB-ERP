...
import type { EmployeeMember, EnrichedAuditLogRow } from "./types"
...
type AuditLogsRpcClient = {
  ...
  ) => Promise<{ data: EnrichedAuditLogRow[] | null; error: unknown }>
}
...
async function getAuditLogsData(): Promise<AuditLogsData> {
  ...
  const dataClient = getServiceRoleClientOrFallback(supabase as SupabaseClient)
  ...
  const logs: AuditLog[] = rows.map((row) => {
    ...
    const rawMetadata = asMetadata(row.metadata)
    const hasEvent = typeof rawMetadata.event === "string" && rawMetadata.event.trim().length > 0
    const metadata = {
      ...rawMetadata,
      ...(hasEvent ? {} : rawAction !== normalizedAction ? { event: rawAction } : {}),
    }
  ...
  // Resolve the affected employee (target) for attendance/leave logs the RPC doesn't enrich.
  const attendanceLeaveTypes = new Set([
    "attendance_record",
    "attendance_record_bulk",
    "attendance_appeal",
    "attendance_exemption",
    "leave_request",
    "leave_request_manual",
  ])
  const affectedIdByLog = new Map<string, string>()
  for (const lg of logs) {
    if (lg.target_user || !attendanceLeaveTypes.has(lg.entity_type)) continue
    const nv = (lg.new_values || {}) as Record<string, unknown>
    const ov = (lg.old_values || {}) as Record<string, unknown>
    let affected: string | undefined
    if (lg.entity_type === "attendance_exemption" && typeof lg.entity_id === "string") affected = lg.entity_id
    if (!affected && typeof nv.user_id === "string") affected = nv.user_id
    if (!affected && typeof ov.user_id === "string") affected = ov.user_id
    if (!affected && Array.isArray(nv.user_ids) && typeof nv.user_ids[0] === "string") affected = nv.user_ids[0]
    if (affected) affectedIdByLog.set(lg.id, affected)
  }
  const affectedIds = [...new Set(affectedIdByLog.values())]
  if (affectedIds.length > 0) {
    const { data: affectedProfilesRaw } = await dataClient
      .from("profiles")
      .select("id, first_name, last_name, company_email, employee_number, department")
      .in("id", affectedIds)
    const affectedProfiles = (affectedProfilesRaw ?? []) as Array<{
      id: string
    ...
    for (const p of affectedProfiles) {
      profMap.set(p.id, {
        ...
        employee_number: p.employee_number ?? undefined,
        department: p.department,
      })
    }
    for (const lg of logs) {
      const aid = affectedIdByLog.get(lg.id)
      if (aid && !lg.target_user) lg.target_user = profMap.get(aid) ?? lg.target_user
    }
  }
  ...
  if (departmentScope) {
    ...
    const logDepartments = rows.flatMap((r) =>
      [r.department, r.user_data?.department, r.target_user_data?.department].filter(Boolean)
    )
